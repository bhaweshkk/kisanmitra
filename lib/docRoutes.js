...
async function getUserFromReq(req) {
  ...
  return await users.findOne({ token }) || null;
}
...
async function handleUpload(req, res) {
  ...
  let record = await docs.findOne({ userId });
  const docEntry = {
    docKey,
    fileName,
  ...
  if (record) {
    ...
    await docs.updateById(record._id, { documents: existingDocs });
  } else {
    // Create new record
    await docs.insert({
      userId,
...
// ── GET /api/docs/my ──────────────────────────────────────────────
async function handleGetMy(req, res) {
  const user = await getUserFromReq(req);
  if (!user) return sendJSON(res, 401, { error: 'Not authenticated' });

  const userId = user._id || user.id;
  const record = await docs.findOne({ userId });
...
async function handleGetFile(req, res, docKey) {
  const user = await getUserFromReq(req);
  if (!user) return sendJSON(res, 401, { error: 'Not authenticated' });
...
async function handleDelete(req, res, docKey) {
  const user = await getUserFromReq(req);
  if (!user) return sendJSON(res, 401, { error: 'Not authenticated' });
  ...
  await docs.updateById(record._id, { documents: existingDocs, status: 'draft', submittedAt: null });
...
async function handleSubmit(req, res) {
  ...
  await docs.updateById(record._id, {
    status:      'pending',
...
// ── GET /api/admin/docs ───────────────────────────────────────────
async function handleAdminGetDocs(req, res) {
  const user = await getUserFromReq(req);
...
async function handleAdminGetFile(req, res, userId, docKey) {
  const user = await getUserFromReq(req);
  if (!user || user.role !== 'admin') return sendJSON(res, 403, { error: 'Admin only' });
...
async function handleAdminUpdateStatus(req, res, userId) {
  ...
  await docs.updateById(record._id, { status, adminNote: adminNote || '' });
