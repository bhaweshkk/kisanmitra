/**
 * KisanMitra server.js — v7 (PostgreSQL permanent storage)
 *
 * MongoDB has been completely removed.
 * All data is stored in PostgreSQL via db.js.
 *
 * ROUTES:
 ...
 *   GROQ_API_KEY=gsk_...         (optional – for AI chat proxy)
 */
'use strict';
...
// ── PostgreSQL (replaces MongoDB completely) ──────────────────────
const { connectDB, collection } = require('./db');

// ── Collections — all backed by PostgreSQL ────────────────────────
...
/** Extract the current user from the Bearer token (stored in users table). */
...
async function requireAuth(req, res) {
  ...
  if (!u) { sendJSON(res, 401, { error: 'Login required. Please sign in.' }); return null; }
  return u;
}

async function requireAdmin(req, res) {
  const u = await getUser(req);
  if (!u || u.role !== 'admin') { sendJSON(res, 403, { error: 'Admin access required.' }); return null; }
  return u;
}
...
async function handleGetGroups(req, res) {
  try {
    const groups = (await groupsCol.find({}))
      .filter(g => !g.status || g.status === 'approved')
      .sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
    sendJSON(res, 200, { groups });
  } catch (e) {
    logger.error('getGroups', { msg: e.message });
    sendJSON(res, 500, { error: 'Could not load groups' });
  }
}
...
async function handleCreateGroup(req, res, body) {
  try {
    if (!body || !body.name || !body.name.trim())
      return sendJSON(res, 400, { error: 'Group name is required' });
    const u = await getUser(req);
    ...
    logger.error('createGroup', { msg: e.message });
    sendJSON(res, 500, { error: 'Could not create group' });
  }
}
...
async function handleGetMessages(req, res, gid) {
  try {
    ...
    sendJSON(res, 500, { error: 'Could not load messages' });
  }
}
...
async function handlePostMessage(req, res, gid, body) {
  try {
    if (!body || !body.content || !body.content.trim())
      return sendJSON(res, 400, { error: 'Message content required' });
    const u = await getUser(req);
    ...
    sendJSON(res, 500, { error: 'Could not post message' });
  }
}
...
async function handleListProducts(req, res) {
  try {
    ...
    logger.error('listProducts', { msg: e.message });
...
async function handleCreateProduct(req, res, body) {
  try {
    ...
    logger.error('createProduct', { msg: e.message });
...
async function handleCreateOrder(req, res, body) {
  try {
    ...
    sendJSON(res, 500, { error: 'Could not place order' });
  }
}
...
async function handleListCompanies(req, res) {
  try {
    ...
    sendJSON(res, 500, { error: 'Could not load companies' });
  }
}
...
async function handleSubmitStory(req, res, body) {
  try {
    ...
    sendJSON(res, 500, { error: 'Could not submit story' });
  }
}
...
async function handleAdminListStories(req, res) {
  try {
    ...
    sendJSON(res, 500, { error: 'Could not load stories' });
  }
}
  ...
  try {
    ...
    if (body.action === 'approve') {
      const company = await companiesCol.insert({
        name:            s.name,
        tagline:         (s.shortDesc || '').slice(0, 80),
        ...
        founders:        s.founders      || [],
        funding:         s.funding       || '',
        farmersImpacted: s.farmersImpacted || '',
        ...
        lessons:         s.lessons       || [],
        forStarters:     s.forStarters   || '',
        story:           body.enrichedStory   || [],
      ...
      await storiesCol.updateById(storyId, {
        status: 'rejected', rejectedAt: Date.now(), reason: body.reason || '',
      });
      sendJSON(res, 200, { message: 'Story rejected' });
    }
  } catch (e) {
    sendJSON(res, 500, { error: 'Could not update story' });
  }
}
...
async function router(req, res, isHttps) {
  ...
  if (pathname === '/api/auth/register' && method === 'POST') {
    ...
    await auth.handleRegister(req, res, b); return;
  }
  if (pathname === '/api/auth/login' && method === 'POST') {
    const b = await parseBody(req).catch(() => null);
    await auth.handleLogin(req, res, b); return;
  }
  if (pathname === '/api/auth/me' && method === 'GET') {
    await auth.handleMe(req, res); return;
  }
  ...
  if (pathname.startsWith('/api/docs/file/') && method === 'GET') {
    await docRoutes.handleGetFile(req, res, pathname.split('/api/docs/file/')[1]); return;
  }
  if (pathname.startsWith('/api/docs/') && method === 'DELETE') {
    await docRoutes.handleDelete(req, res, pathname.split('/').pop()); return;
  }
  if (pathname === '/api/admin/docs' && method === 'GET') { await docRoutes.handleAdminGetDocs(req, res); return; }
  if (pathname.match(/^\/api\/admin\/docs\/[^/]+\/status$/) && method === 'PATCH') {
    const b = await parseBody(req).catch(() => null);
    await docRoutes.handleAdminUpdateStatus(req, res, pathname.split('/')[4]); return;
  }
  if (pathname.match(/^\/api\/admin\/docs\/[^/]+\/file\/[^/]+$/) && method === 'GET') {
    const p = pathname.split('/');
    await docRoutes.handleAdminGetFile(req, res, p[4], p[6]); return;
  }

  // ── Admin ─────────────────────────────────────────────────────────
  if (pathname === '/admin' && method === 'GET') {
    serveStatic({ url: '/admin.html', method: 'GET', headers: req.headers }, res, CONFIG.PUBLIC_DIR); return;
  }
  ...
  if (pathname.startsWith('/api/admin/contacts/') && method === 'DELETE') {
    await adminRoutes.handleDeleteContact(req, res, pathname.split('/').pop()); return;
  }
  if (pathname === '/api/admin/verify-password' && method === 'POST') {
    const b = await parseBody(req).catch(() => null);
    adminRoutes.handleVerifyPassword(req, res, b); return;
  }
  ...
  // ── Community Groups (PostgreSQL-backed) ──────────────────────────
  if (pathname === '/api/groups' && method === 'GET')  { await handleGetGroups(req, res); return; }
  if (pathname === '/api/groups' && method === 'POST') {
    const b = await parseBody(req).catch(() => null);
    await handleCreateGroup(req, res, b); return;
  }
  if (pathname.match(/^\/api\/groups\/[^/]+\/messages$/) && method === 'GET') {
    await handleGetMessages(req, res, pathname.split('/')[3]); return;
  }
  if (pathname.match(/^\/api\/groups\/[^/]+\/messages$/) && method === 'POST') {
    const b = await parseBody(req).catch(() => null);
    await handlePostMessage(req, res, pathname.split('/')[3], b); return;
  }

  // ── Marketplace (PostgreSQL-backed) ───────────────────────────────
  if (pathname === '/api/marketplace/products'    && method === 'GET')  { await handleListProducts(req, res); return; }
  if (pathname === '/api/marketplace/products'    && method === 'POST') {
    const b = await parseBody(req).catch(() => null);
    await handleCreateProduct(req, res, b); return;
  }
  ...
  if (pathname === '/api/marketplace/orders'      && method === 'POST') {
    const b = await parseBody(req).catch(() => null);
    await handleCreateOrder(req, res, b); return;
  }

  // ── AgriInnovate (PostgreSQL-backed) ──────────────────────────────
  if (pathname === '/api/innovate/companies' && method === 'GET') { await handleListCompanies(req, res); return; }
  if (pathname === '/api/innovate/stories'   && method === 'POST') {
    const b = await parseBody(req).catch(() => null);
    await handleSubmitStory(req, res, b); return;
  }
  ...
  if (pathname.match(/^\/api\/innovate\/stories\/[^/]+$/) && method === 'PATCH') {
    const b = await parseBody(req).catch(() => null);
    await handleAdminApproveStory(req, res, pathname.split('/').pop(), b); return;
  }
  ...
  if (pathname.startsWith('/api/plugins/')) {
    let b = null;
    if (method === 'POST') {
      try { b = await parseBody(req); } catch (e) { sendJSON(res, 400, { error: e.message }); return; }
    }
  ...
  if (pathname.startsWith('/api/')) {
    sendJSON(res, 404, { error: `Not found: ${method} ${pathname}` }); return;
  }
...
validate(CONFIG).then(async () => {
  ...
  httpServer.on('error', err => {
    ...
    if (err.code === 'EADDRINUSE') {
      logger.error(`Port ${CONFIG.PORT} already in use. Change PORT in .env`);
      process.exit(1);
    }
  });
  ...
  if (CONFIG.ENABLE_HTTPS) {
    try {
      ...
      trackConn(hs);
      hs.listen(CONFIG.HTTPS_PORT, CONFIG.HOST, () =>
  ...
  logger.info('══════════════════════════════════════════════════════');
  logger.info('   🌾  KisanMitra v7 — PostgreSQL edition!');
  logger.info(`   Website   : http://localhost:${CONFIG.PORT}`);
  logger.info(`   Admin     : http://localhost:${CONFIG.PORT}/admin`);
  logger.info(`   Health    : http://localhost:${CONFIG.PORT}/api/health`);
  logger.info(`   Database  : ${process.env.DATABASE_URL ? '✅ PostgreSQL connected' : '⚠️  DATABASE_URL not set!'}`);
  logger.info('══════════════════════════════════════════════════════');
...
process.on('uncaughtException',  e => {
  logger.error('Uncaught exception', { msg: e.message, stack: e.stack });
  process.exit(1);
});
