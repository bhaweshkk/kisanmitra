...
const companiesCol = db.collection('innovate_companies');
const ideasCol     = db.collection('innovate_ideas');
...
async function handleAdminAddCompany(req, res, body) {
  try {
    ...
    const company = companiesCol.insert({
      ...
      founders:          Array.isArray(body.founders) ? body.founders : (body.founders||'').split(',').map(s=>s.trim()).filter(Boolean),
      funding:           body.funding  || '',
      farmersImpacted:   body.farmersImpacted || '',
      ...
      authenticityScore: parseInt(body.authenticityScore) || 85,
      auditor:           body.auditor  || 'KisanMitra Team',
...
async function router(req, res, isHttps) {
  ...
  if (pathname==='/api/docs/my'     &&method==='GET')   { docRoutes.handleGetMy(req,res); return; }
  if (pathname==='/api/docs/submit' &&method==='POST')  { await docRoutes.handleSubmit(req,res); return; }
  if (pathname.startsWith('/api/docs/file/')&&method==='GET') { docRoutes.handleGetFile(req,res,pathname.split('/api/docs/file/')[1]); return; }
  if (pathname.startsWith('/api/docs/')&&method==='DELETE') { docRoutes.handleDelete(req,res,pathname.split('/').pop()); return; }
  if (pathname==='/api/admin/docs'  &&method==='GET')   { docRoutes.handleAdminGetDocs(req,res); return; }
  ...
  if (pathname.match(/^\/api\/admin\/docs\/[^/]+\/file\/[^/]+$/)&&method==='GET') { const p=pathname.split('/'); docRoutes.handleAdminGetFile(req,res,p[4],p[6]); return; }
  ...
  if (mp) {
    ...
    if (pathname.match(/^\/api\/marketplace\/products\/[^/]+$/) && method==='DELETE') { mp.deleteProduct(req,res,pathname.split('/').pop()); return; }
    if (pathname==='/api/marketplace/my-listings'       && method==='GET')    { mp.myListings(req,res); return; }
    if (pathname==='/api/marketplace/orders/mine'       && method==='GET')    { mp.myOrders(req,res); return; }
    if (pathname==='/api/marketplace/rzp-key'           && method==='GET')    { mp.getRzpKey(req,res); return; }
    if (pathname==='/api/marketplace/payment/create'    && method==='POST')   { const b=await parseBody(req).catch(()=>null); await mp.createPayment(req,res,b); return; }
    if (pathname==='/api/marketplace/payment/verify'    && method==='POST')   { const b=await parseBody(req).catch(()=>null); await mp.verifyPayment(req,res,b); return; }
  }
