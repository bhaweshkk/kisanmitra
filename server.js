...
async function router(req, res, isHttps) {
  ...
  if (pathname.startsWith('/api/docs/file/') && method === 'GET') {
    const docKey = pathname.split('/api/docs/file/')[1];
    docRoutes.handleGetFile(req, res, docKey); return;
  }
  if (pathname.startsWith('/api/docs/') && method === 'DELETE') {
    const docKey = pathname.split('/').pop();
    docRoutes.handleDelete(req, res, docKey); return;
  }
  ...
  if (pathname.match(/^\/api\/admin\/docs\/[^/]+\/status$/) && method === 'PATCH') {
    ...
    await docRoutes.handleAdminUpdateStatus(req, res, userId); return;
  }
  if (pathname.match(/^\/api\/admin\/docs\/[^/]+\/file\/[^/]+$/) && method === 'GET') {
    const parts = pathname.split('/');
    const userId = parts[4]; const docKey = parts[6];
    docRoutes.handleAdminGetFile(req, res, userId, docKey); return;
  }
